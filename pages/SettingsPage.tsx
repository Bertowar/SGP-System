...
const SettingsPage: React.FC = () => {
    ...
    const handleGlobalSave = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (result.fallback) {
                setMsg('Aviso: Parâmetros básicos salvos, mas NOVOS CAMPOS foram ignorados. Schema do Banco desatualizado. Execute o script de migração!');
            } else {
                ...
                <div className="space-y-6">
                    ...
                    <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-6">
                        ...
                        <div className="grid grid-cols-2 gap-6 mb-4">
                            ...
                            />
                            <Input
                                label="Limite Aparas Extrusão (%)"
                                type="number"
                                step="0.1"
                                placeholder="5.0"
                                value={settings.extrusionScrapLimit}
                                onChange={e => setSettings({ ...settings, extrusionScrapLimit: Number(e.target.value) })}
                            />
                            <Input
                                label="Limite Aparas TFs (%)"
                                type="number"
                                step="0.1"
                                placeholder="2.0"
                                value={settings.thermoformingScrapLimit}
                                onChange={e => setSettings({ ...settings, thermoformingScrapLimit: Number(e.target.value) })}
                            />
                        </div>
