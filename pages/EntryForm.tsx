...
import { Save, AlertCircle, Loader2, ArrowLeft, Clock, Cpu, Play, Square, AlertTriangle, User, ChevronRight, X, Package, Timer, Scale, PowerOff, Calendar, Beaker, FlaskConical, Droplet, ClipboardList } from 'lucide-react';
import { formatError } from '../services/storage';
...
const EntryForm: React.FC = () => {
  ...
  useEffect(() => {
      if (isExtrusion) {
          const r = parseFloat(refileQty) || 0;
          const b = parseFloat(borraQty) || 0;
          const totalLoss = r + b;
  ...
  useEffect(() => {
    if (editEntry) {
        ...
        setQtyOK(editEntry.qtyOK.toString());
        setQtyDefect(editEntry.qtyDefect.toString());
        setScrapReasonId(editEntry.scrapReasonId || '');
        setObservations(editEntry.observations);
        setCycleRate(editEntry.cycleRate?.toString() || editEntry.metaData?.cycleRate?.toString() || '');
        ...
        const weight = editEntry.measuredWeight || editEntry.metaData?.measuredWeight || editEntry.metaData?.peso_produto || '';
        setMeasuredWeight(weight.toString());

        // Load Extrusion Specifics
        if (editEntry.metaData?.extrusion) {
            const ext = editEntry.metaData.extrusion;
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
        ...
        const weightValue = Number(measuredWeight.toString().replace(',', '.')) || 0;
        const boxesValue = isExtrusion ? Number(qtyOK) : 1; // Para bobinas, OK é a bobina
        ...
        if (isExtrusion && !isDowntime) {
            ...
            Object.keys(additives).forEach((k) => {
                ...
                if (val) additivesPayload[k] = parseFloat(val);
            });
            metaPayload.extrusion = {
                mix: mixItems.filter(i => i.type && i.qty),
                additives: additivesPayload,
                refile: parseFloat(refileQty) || 0,
                borra: parseFloat(borraQty) || 0
            };
        ...
        const entry: ProductionEntry = {
            ...
            startTime,
            endTime,
            qtyOK: isDowntime ? 0 : (isExtrusion ? weightValue : Number(qtyOK || 0)),
            qtyDefect: isDowntime ? 0 : Number(qtyDefect || 0),
            ...
            observations,
            cycleRate: cycleRate ? Number(cycleRate) : 0,
                    ...
                    <div className="overflow-y-auto p-6 bg-white flex-1">
                        ...
                        <form id="entry-form" onSubmit={handleSubmit}>
                            <div className="mb-6 p-5 bg-slate-50 rounded-xl border border-slate-100">
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-12 gap-6 items-start">
                                    {!isDowntime && availableOps.length > 0 && (
                                        <div className="col-span-1 md:col-span-12 bg-blue-50/50 p-3 rounded-lg border border-blue-100">
                                            <label className="text-sm font-bold text-blue-800 mb-1 flex items-center"><ClipboardList size={16} className="mr-2" /> Vincular Ordem de Produção (OP)</label>
                                            <select className="w-full px-3 py-2 border border-blue-200 rounded-lg text-sm bg-white focus:ring-2 focus:ring-blue-400 font-medium text-slate-700" value={selectedOpId} onChange={e => handleOpChange(e.target.value)}>
                                                <option value="">- Sem vínculo (Avulso) -</option>{availableOps.map(op => <option key={op.id} value={op.id}>{op.id} - {op.product?.produto || `Cod ${op.productCode}`} (Qtd: {op.targetQuantity})</option>)}
                                            </select>
                                        </div>
                                    )}
                                    <div className="col-span-1 md:col-span-3"><div className="flex flex-col space-y-1"><label className="text-sm font-semibold text-slate-700">Data</label><input type="date" className="px-3 py-2 border rounded-lg h-[42px] font-bold text-slate-800" value={date} onChange={e => setDate(e.target.value)} max={today} required /></div></div>
                                    {isDowntime && (
                                        <div className="col-span-1 md:col-span-3 flex items-center h-[42px] mt-[26px]">
                                            <div className="flex items-center bg-orange-50 border border-orange-200 px-3 py-2 rounded-lg w-full h-full cursor-pointer hover:bg-orange-100 transition-colors" onClick={() => { const newVal = !isLongStop; setIsLongStop(newVal); if(newVal) { setOperatorId(''); const s = shifts.find(s => s.name === shift); if(s) { setStartTime(s.startTime); setEndTime(s.endTime); } } }}>
                                    ...
                                    <div className={`col-span-1 ${isDowntime ? 'md:col-span-6' : 'md:col-span-3'}`}><label className="text-sm font-semibold text-slate-700 mb-1 block">Turno *</label><select className="w-full px-3 py-2 border rounded-lg h-[42px] bg-white" value={shift} onChange={handleShiftChange} required disabled={!machineId}><option value="">Selecione...</option>{availableShifts.map(s => <option key={s.id} value={s.name}>{s.name}</option>)}</select></div>
                                    {!isDowntime && (<div className="col-span-1 md:col-span-6 grid grid-cols-2 gap-4"><Input label="Início" type="time" value={startTime} onChange={e => setStartTime(e.target.value)} required /><Input label="Fim" type="time" value={endTime} onChange={e => setEndTime(e.target.value)} required /></div>)}
                                    <div className="col-span-1 md:col-span-6"><label className="text-sm font-semibold text-slate-700 mb-1 block">Operador {isOperatorExempt ? '(Opcional)' : '*'}</label><select className="w-full px-3 py-2 border rounded-lg h-[46px] bg-white" value={operatorId} onChange={e => setOperatorId(Number(e.target.value))} required={!isOperatorExempt} disabled={isOperatorExempt && isLongStop}><option value="">{isOperatorExempt ? 'Máquina sem operador' : 'Selecione...'}</option>{filteredOperators.map(op => <option key={op.id} value={op.id}>{op.name}</option>)}</select></div>
                                    {!isDowntime ? (<div className="col-span-1 md:col-span-6 relative">{selectedOpId && (<div className="absolute inset-0 bg-slate-100/50 z-10 cursor-not-allowed rounded-lg border border-transparent hover:border-slate-300" title="Produto definido pela OP." onClick={() => alert("Produto travado pela OP selecionada.")}></div>)}<ProductSelect products={filteredProducts} value={productCode} onChange={setProductCode} error={!productCode && errorMsg ? 'Selecione' : undefined} /></div>) : (<div className="col-span-1 md:col-span-6"><label className="text-sm font-semibold text-slate-700 mb-2 block">Motivo Parada *</label><select className="w-full px-3 py-3 border rounded-lg h-[46px] bg-white font-bold" value={downtimeTypeId} onChange={e => setDowntimeTypeId(e.target.value)} required><option value="">Selecione...</option>{downtimeTypes.map(d => <option key={d.id} value={d.id}>{d.description}</option>)}</select></div>)}
                                </div>
                            </div>
                            ...
                            <div>
                                        ...
                                        <div className="grid grid-cols-1 md:grid-cols-4 gap-4 p-4 bg-slate-50 rounded-xl border border-slate-100">
                                            {isExtrusion ? (
                                                <>
                                                    ...
                                                    <Input label="Qtd Bobinas (Un)" type="number" value={qtyOK} onChange={e => setQtyOK(e.target.value)} placeholder="0" className="font-bold text-2xl h-12 text-green-700" />
                                                    <Input label="Ciclagem" type="number" value={cycleRate} onChange={e => setCycleRate(e.target.value)} placeholder="0.0" className="font-bold text-2xl h-12 text-slate-700" />
                                                </>
                                            ) : (
                                                <>
                                            ...
                                            <div className="md:col-span-1">{isExtrusion ? (<div className="flex gap-2"><Input label="Refile (Kg)" type="number" value={refileQty} onChange={e => setRefileQty(e.target.value)} placeholder="0" className="border-orange-200 text-orange-700 font-bold" /><Input label="Borra (Kg)" type="number" value={borraQty} onChange={e => setBorraQty(e.target.value)} placeholder="0" className="border-red-200 text-red-700 font-bold" /></div>) : (<Input label="Qtd Refugo" type="number" value={qtyDefect} onChange={e => setQtyDefect(e.target.value)} placeholder="0" className="font-bold text-2xl h-12 text-red-600 border-red-200 focus:border-red-500" />)}</div>
                                        </div>
                                ...
                                <div className="mt-8 space-y-4 border-t border-slate-100 pt-6"><Textarea label="Observações" value={observations} onChange={e => setObservations(e.target.value)} placeholder="Ocorrências, detalhes..." />{filteredCustomFields.length > 0 && (<div className="bg-slate-50 p-5 rounded-xl border border-slate-200"><h4 className="text-xs font-bold text-slate-500 uppercase tracking-wider mb-4 flex items-center"><Clock size={14} className="mr-2" /> Dados Adicionais</h4><DynamicFields fields={filteredCustomFields} values={customValues} onChange={(k, v) => setCustomValues(prev => ({ ...prev, [k]: v }))} /></div>)}</div>
                            </div>
                        </form>
                    </div>
