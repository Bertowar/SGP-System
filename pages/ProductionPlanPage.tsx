...
import { fetchMaterials, fetchBOM } from '../services/inventoryService';
...
const ProductionPlanPage: React.FC = () => {
    ...
    // --- LOGIC: MRP Availability Check ---
    useEffect(() => {
        const checkAvailability = async () => {
            if (!editingOrder.productCode || !editingOrder.targetQuantity || !modalOpen) {
                setAvailability([]);
                return;
            }
            ...
            try {
                ...
                const bomItems = await fetchBOM(editingOrder.productCode);
                ...
                const needed = bomItems.map(item => {
                    // Calculate Total Required
                    // If item.quantityRequired is per unit
                    const totalReq = (item.quantityRequired || 0) * (editingOrder.targetQuantity || 0);

                    // Find current stock
                    // We try to find in our full inventory list first for latest data, fallback to item.material if provided
                    const freshMat = inventory.find(m => m.id === item.materialId);
                    const currentStock = freshMat ? freshMat.currentStock : (item.material?.currentStock || 0);
                    const matName = freshMat ? freshMat.name : (item.material?.name || 'Material Desconhecido');
                    const Unit = freshMat ? freshMat.unit : (item.material?.unit || 'kg');
                    ...
                    return {
                        ...
                        unit: Unit,
                        status: currentStock >= totalReq ? 'AVAILABLE' : 'INSUFFICIENT',
                        missing: Math.max(0, totalReq - currentStock)
                    };
                });

                setAvailability(needed);
            } catch (e) {
                console.error("MRP Error", e);
            } finally {
                setCheckingMRP(false);
            }
        };
    ...
    }, [editingOrder.productCode, editingOrder.targetQuantity, modalOpen, inventory]);
                            ...
                            <div className="pl-1 mb-5">
                                ...
                                <div className="h-6 w-full bg-blue-50 rounded-full overflow-hidden border border-blue-100 relative">
                                    <div
                                        ...
                                        style={{ width: `${Math.max(5, getProgress(order.producedQuantity, order.targetQuantity))}%` }}
                                    ></div>
