-- MIGRATION: 20251228_fix_dashboard_aggregation.sql
-- DESCRIPTION: Adds aggregation logic for Refile, Borra, and Aparas (Return/Loss) to get_dashboard_metrics.
...
FROM (
        SELECT pe.machine_id,
            ...
            -- Aggregation Update: Return (Refile/Aparas) and Loss (Borra)
            SUM(
                CASE
                    -- Extrusion: Refile is in meta_data
                    WHEN m.sector = 'Extrusão' THEN COALESCE(
                        (pe.meta_data->'extrusion'->>'refile')::numeric,
                        0
                    ) -- Thermoforming: Aparas = Measured - Theoretical
            ...
            SUM(
                CASE
                    -- Extrusion: Borra is in meta_data
                    WHEN m.sector = 'Extrusão' THEN COALESCE(
                        (pe.meta_data->'extrusion'->>'borra')::numeric,
                        0
                    ) -- Others: Default to recorded defect qty
                    ELSE pe.qty_defect
                END
            ) as total_loss
        FROM public.production_entries pe
